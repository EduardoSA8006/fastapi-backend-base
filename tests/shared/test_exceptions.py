...
def test_handler_converte_not_found_em_404_padronizado() -> None:
    client = client_with_raising_route(NotFoundError("Recurso não encontrado."))
...
def test_handler_converte_conflict_em_409() -> None:
    client = client_with_raising_route(ConflictError("Já existe."))
    response = client.get("/_boom")
...
def test_handler_converte_unavailable_em_503() -> None:
    client = client_with_raising_route(UnavailableError("Dependência fora."))
    response = client.get("/_boom")
...
def test_handler_usa_detail_default_da_classe() -> None:
    ...
    client = client_with_raising_route(NotFoundError())
    response = client.get("/_boom")
    assert response.status_code == 404
...
def test_unavailable_e_logado_como_error(
    ...
    client = client_with_raising_route(UnavailableError("MinIO fora do ar."))
...
def test_not_found_nao_gera_log_de_erro(caplog: Any) -> None:
    ...
    client = client_with_raising_route(NotFoundError("x"))
